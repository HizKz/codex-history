...
![合成データで表示したcodex-historyの会話一覧・本文・ターン差分](docs/assets/overview.svg)

## 主な機能

- 会話一覧・ターン単位トランスクリプト・選択ターンのDiffを、端末幅に応じて
  3ペイン・2ペイン・1ペインで表示
...
| キー | 操作 |
| --- | --- |
| `j` / `k`、矢印 | 会話を移動・本文やDiffを縦スクロール |
| `ctrl+u` / `ctrl+d` | 本文・Diff・詳細を半画面スクロール |
| `h` / `l`、左右キー | 長いDiff行を横スクロール |
| `[` / `]` | 前後のターンへ移動 |
| `tab` | ペイン切り替え |
| `/` | 全文検索 |
...
SQLiteインデックスには、コマンド出力、MCPの引数・結果、ファイル変更のパス、
Diff本文などの展開式ツール詳細を含めません。3文字以上の検索では
trigramインデックスを使って関連度と一致箇所を表示し、1〜2文字ではリテラルな
部分一致を維持します。永続キャッシュを使いたくない場合は `--no-cache` を
指定できます。
